...
- [-] Mobile: dash button + double-jump via double-tap jump (JumpRequest, works on touch/keyboard/gamepad) — see [mobile-dash-doublejump.md](mobile-dash-doublejump.md)

- [x] FIX: wall decor is on the OUTSIDE face of the walls (faces the park) → invisible from inside — flip every wall's decor + SurfaceGuis to the INTERIOR (court-facing) side; re-author + verify visually from inside — see [wall-decoration-gym-playful-fades.md](wall-decoration-gym-playful-fades.md) — DONE: root cause = `buildWallDecor` pushed decor the WRONG way off the wall plane (`wallPlane - face*off` buried it on the exterior/park side, behind the opaque wall) AND the N/S SurfaceGui `Face` was swapped (Roblox NormalId.Front=−Z, Back=+Z, so WallN's +Z-interior gui used Front→faced the park, WallS's −Z-interior used Back→faced the park). Fix: flip all 5 placement offsets to `wallPlane + face*off` (push INTO the room toward the court); WallN scoreboard Front→Back, WallS banners Back→Front (E/W Left/Right were already correct); SurfaceGui LightInfluence 0.3→0 (self-lit so bright gym PointLights/bloom don't wash the text out). Re-authored the persisted Gym (destroyed the 4 wrong-side WallDecor folders, re-ran buildWallDecor via a fresh-load module clone to dodge the stale `require` cache, origin derived from the live wall positions = (0,0,200)). Verified IN PLAY from the court: scoreboard "9 SQUARE / REC CENTER", clock, bunting, padding (WallN); "2025/2024/CHAMPIONS" banners (WallS); color-block mural + posters + "TEAM SPIRIT" (E/W) all READABLE facing the players; decor still fades WITH its wall when the eye crosses outside (no floating decor); 48 parts CanCollide=false, out of NineSquare.Frame. Studio left in Edit.
...
- [x] No double-hit: same player can't hit the ball twice in a row → fault + ragdoll out until the point resets — see [no-double-hit-ragdoll-penalty.md](no-double-hit-ragdoll-penalty.md) — DONE: server tracks the previous player touch (lastHitterId via rotation:occupantAtCell off bc.onPlayerHit); same occupant striking twice with no other occupant in between → fault on their own cell (reuses attributeFault → deferred-rotation-at-rest → re-serve) + HUMAN ragdoll (PlatformStand + re-enabled FallingDown/Ragdoll + flop impulse), recovered on the re-serve placeAtCell. Bots fault, no ragdoll. Tracker resets each serve; rallyArmed/resolvingRally/pendingFaultRank guards = one fault, no wedge. Tunables in GridConfig.doubleHit*. Deployed to Studio + verified in Play (forced bot + human double-hits: fault + rotation + ragdoll + recovery; normal alternating rallies never false-fault; loop never wedges).
